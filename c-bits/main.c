...
int main(int argc, char** argv) {
	...
	int* HEAP = NULL;
	int rc = posix_memalign((void*) &HEAP, 1 << 3, HEAP_SIZE * sizeof(int));
	if (rc != 0) {
		switch (rc) {
			case EINVAL: fprintf(stderr, "Alignment is not a power of two multiple of %d\n", sizeof(void*)); break;
			case ENOMEM: fprintf(stderr, "Insufficient memory\n"); break;
			default:     fprintf(stderr, "posix_memalign returned %d\n", rc); break;
		}
		exit(1);
	}
