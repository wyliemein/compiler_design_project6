module Language.Fox.Label (tag, tails) where
...
tailsE g = go True
  where
    ...
    go _ (Tuple es l)      = tailsTop l (Tuple es')
      where
        es'                = go False <$> es

    go _ (GetItem et ei l) = tailsTop l (GetItem et' ei')
      where
        et'                = go False et
        ei'                = go False ei
...
tagE = go
  where
    ...
    go i (Tuple es l)      = tagTop i' l (Tuple es')
      where
        (i', es')          = L.mapAccumL go i es

    go i (GetItem et ei l) = tagTop i'' l (GetItem et' ei')
      where
        (i',  et')         = go i  et
        (i'', ei')         = go i' ei
